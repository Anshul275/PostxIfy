...
const { registerValidation, loginValidation, forgotPassValidation, resetPassValidation } = require("./validations/validate");
...
router.post("/register", async (req, res) => {
    ...
    try {
        ...
        if (userIdExist) {
            return res.status(409).json({ message: "Provided user_id already taken..." });
        }

        const emailExist = await User.findOne({ email: req.body.email });
        if (emailExist) {
            return res.status(409).json({ message: "Provided email_id already taken..." });
        }
        ...
        res.status(201).json({ message: user.userId + " Registered Successfully..." });
    } catch (err) {
...
router.post("/forgotPassword", async (req, res) => {
    ...
    try {
        ...
        html_data += "OTP to reset your TrashHUB account password is : <b>" + password + "</b>\n";
