...
router.patch("/:postId", verifyPostOpn, async (req, res) => {
    ...
    try {
        ...
        if (post) {
            ...
            res.status(400).json({
                message: "No POST by this post id"
            });
        }
    } catch (err) {
        res.status(500).json({
...
router.patch("/like/:postId", verifyPostOpn, async (req, res) => {
    const {
        error
    } = like_unlike_PostValidation(req.body);
    if (error) return res.status(400).json({
        message: error.details[0].message
    });
    try {
        ...
        if (find) {
            if (find.likedBy.includes(req.body.userId)) {
                return res.status(400).json({
                    message: "Access Denied! Already Liked"
                });
            }
            const updatedPost = await Post.updateOne({
                _id: req.params.postId
...
router.patch("/unlike/:postId", verifyPostOpn, async (req, res) => {
    ...
    try {
        ...
        if (find) {
            if (!find.likedBy.includes(req.body.userId)) {
                return res.status(400).json({
                    message: "Access Denied! Already Unliked"
                });
            }
            const updatedPost = await Post.updateOne({
                _id: req.params.postId
